...
class recipe(object):
    def __init__(self):
        ...
        self.LeftHandList = ["Lettuce", "Tomato", "Cucumber", "Eggplant",
                             "cheese", "Cheese_sliced", "Dough", "Onion", "Potato"]
        ...
        self.IngredList = ["Lettuce", "Tomato", "Cucumber", "Eggplant", "Onion", "cheese", "Cheese_sliced", "Dough",
                           ...
                           "Salami", "Potato"]
        self.ContList = ["Fridge", "Plate", "Plate2", "Hand",
                         "BreadBP", "Dough", "Pot", "Board", "Cup", "SauceBottle"]
        self.CutableList = ["Lettuce", "Tomato", "Cucumber", "Eggplant", "Onion", "Potato", "Lemon", "Mango", "Kiwi",
                            "Peach", "Apple", "Orange"]
        self.PeelableList = ["Apple", "Cucumber",
                             "Kiwi", "Mango", "Orange", "Peach", "Potato"]
        self.JuiceList = ["Cucumber", "Tomato", "Apple",
                          "Kiwi", "Mango", "Orange", "Peach", "Lemon"]
    ...
    def InitDict(self):
        for obj in self.IngredList:
            self.ObjDict[obj] = {"Pos": "Fridge", "Cut": False,
                                 "Peel": False, "Juice": False, "Cook": False}
        self.ObjDict['Sauce'] = {
            "Pos": "SauceBottle", "Cut": False, "Peel": False, "Juice": False, "Cook": False}
  ...
  # 						os.remove("static/img/temp.jpg")
    ...
    def Use(self, tool, folder_name="", count=[0], f_label="", f_fluent=""):
        ...
        if tool == "Knife":
            ...
            self.a = self.env.MoveContactToObject(
                hand, "ProcMesh", "Board", "Box")
            self.a = self.env.ReleaseObject(hand)
            self.a = self.env.MoveToNeutral(hand)
            self.a = self.env.MoveToObject("RightHand", "Knife", "grabpoint")
            self.a = self.env.GrabObject(
                "RightHand", "Knife", "StaticMeshComponent0")
            ...
            self.a = self.env.GrabObject(
                "LeftHand", "Peeler", "StaticMeshComponent0")
            self.a = self.env.MoveToNeutral("LeftHand")
            self.a = self.env.MoveContactToObject(
                "LeftHand", "Blade", actor_in_hand, "Skin")
            self.a = self.env.MoveToNeutral("LeftHand")
            self.a = self.env.MoveContactToObject(
                "LeftHand", "grabpoint", "282Main3", "Box")
            ...
            self.a = self.env.GrabObject(
                "RightHand", "grater", "StaticMeshComponent0")
            self.a = self.env.MoveToNeutral("RightHand")
            self.a = self.env.MoveContactToObject(
                "RightHand", "StaticMeshComponent0", actor_in_hand, "ProcMesh")
            self.a = self.env.MoveToNeutral("RightHand")
            self.a = self.env.MoveContactToObject(
                "RightHand", "StaticMeshComponent0", "284Main_2", "Box")
            self.a = self.env.ReleaseObject("RightHand")
            self.a = self.env.MoveToNeutral("RightHand")
            self.a = self.env.MoveContactToObject(
                "LeftHand", "ProcMesh", "284Main_2", "Box1")
            self.a = self.env.ReleaseObject("LeftHand")
            self.a = self.env.MoveToNeutral("LeftHand")
            ...
            self.a = self.env.MoveToObject(
                "RightHand", "SauceBottle", "StaticMeshComponent0")
            self.a = self.env.GrabObject(
                "RightHand", "SauceBottle", "StaticMeshComponent0")
            self.a = self.env.MoveToNeutral("RightHand")
            self.a = self.env.MoveContactToObject(
                "RightHand", "StaticMeshComponent0", "Plate2", "Box2")
            self.a = self.env.step("RightHandTwistLeft", scale=6)
            self.a = self.env.step("RightHandTwistRight", scale=6)
            self.a = self.env.MoveToNeutral("RightHand")
            self.a = self.env.MoveContactToObject(
                "RightHand", "StaticMeshComponent0", "281Main_5", "Box")
            self.a = self.env.ReleaseObject("RightHand")
            self.a = self.env.MoveToNeutral("RightHand")
            for thing in self.ObjDict:
            ...
            self.a = self.env.MoveContactToObject(
                hand, "ProcMesh", "Juicer", "SqueezeBox")
            self.a = self.env.ReleaseObject(hand)
            self.a = self.env.MoveToNeutral(hand)
