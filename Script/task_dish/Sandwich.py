...
class Sandwich():
	def __init__(self, name="Sandwich", sub_goal=[None], goal_states=[None], max_steps=10000, frame_size=84, frame_skip=1):
		self.steps = 0
	...
	def step(self, a, sub_goal=[], rollout=False):
		...
		# print(data['objects_vis']['SauceBottle'])
...
def test():
	task_list = [7,0,2,4,8,8,9,6,7,0,10,8,8,9,6,7,0,5,8,1,3,5,8,9,6]
	task_fn = lambda : Sandwich(frame_skip=1, max_steps=100)
	task = task_fn()
	task.start()
	# task.reset()
	for a in task_list:
