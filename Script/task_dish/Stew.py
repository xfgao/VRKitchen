...
class Stew():
	def __init__(self, name="Stew", sub_goal=[None], goal_states=[None], max_steps=10000, frame_size=84, frame_skip=1):
		self.steps = 0
...
def test():
	task_list = [9,0,3,7,10,2,5,7,10,1,8,9,9,0,3,6,9,9,1,8,4]
	task_fn = lambda : Stew(frame_skip=1, max_steps=100)
	task = task_fn()
	task.start()
	# task.reset()
	for a in task_list:
